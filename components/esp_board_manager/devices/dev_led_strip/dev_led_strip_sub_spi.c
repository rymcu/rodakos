...
static const char *TAG = "DEV_LED_STRIP_SPI";

int dev_led_strip_sub_spi_init(void *cfg, int cfg_size, void **device_handle)
{
    ...
    if (handles == NULL) {
        ESP_LOGE(TAG, "Failed to allocate SPI LED strip handle");
        return -1;
    }

    esp_err_t ret = led_strip_new_spi_device(&config->strip_config,
                                             &config->sub_cfg.spi.spi_config,
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to clear SPI LED strip: %s", esp_err_to_name(ret));
...
int dev_led_strip_sub_spi_deinit(void *device_handle)
{
    ...
    if (handles->strip_handle) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to delete SPI LED strip: %s", esp_err_to_name(ret));
            return -1;
        }
        handles->strip_handle = NULL;
    }
