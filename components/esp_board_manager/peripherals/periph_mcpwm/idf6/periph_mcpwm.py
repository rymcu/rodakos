...
def parse(name: str, config: dict) -> dict:
    ...
    try:
        ...
        if io_loop_back:
            print(f"YAML WARNING: 'io_loop_back' flag in MCPWM peripheral '{name}' is deprecated in IDF >= 6.0 and will be ignored.")
            ...
            'struct_init': {
                ...
                'generator_config': {
                    ...
                    'flags': {
                        ...
                        # 'io_loop_back': 1 if io_loop_back else 0, // Removed in V2
                        # 'io_od_mode': 1 if io_od_mode else 0, // Removed in V2
                        # 'pull_up': 1 if pull_up else 0, // Removed in V2
                        # 'pull_down': 1 if pull_down else 0 // Removed in V2
                    }
                },
                'extra_flags': {
                    'invert_pwm': 1 if invert_pwm else 0,
        ...
        raise ValueError(f"YAML validation error in MCPWM V2 peripheral '{name}': {e}")
    except Exception as e:
        # Catch any other exceptions and provide context
        raise ValueError(f"Error parsing MCPWM V2 peripheral '{name}': {e}")
