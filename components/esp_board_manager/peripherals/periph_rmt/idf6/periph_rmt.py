...
def parse(name: str, full_config: dict, peripherals_dict=None) -> dict:
    ...
    if role_l == 'tx':
        ...
        tx_config = {
            ...
            'clk_src': tx_cfg.get('clk_src', 'RMT_CLK_SRC_DEFAULT'),
            ...
            'flags': {
                ...
                # 'io_loop_back': bool(tx_cfg.get('flags', {}).get('io_loop_back', False)), // Removed in V2
                # 'io_od_mode': bool(tx_cfg.get('flags', {}).get('io_od_mode', False)), // Removed in V2
        ...
        if io_loop_back:
            print(f"YAML WARNING: 'io_loop_back' flag in RMT peripheral '{name}' is deprecated in IDF >= 6.0 and will be ignored.")

        channel_config = {'tx': tx_config}

        extra_flags = {
            'io_od_mode': 1 if bool(tx_cfg.get('flags', {}).get('io_od_mode', False)) else 0,
        }
        ...
        rx_config = {
            ...
            'clk_src': rx_cfg.get('clk_src', 'RMT_CLK_SRC_DEFAULT'),
            ...
            'flags': {
                ...
                # 'io_loop_back': bool(rx_cfg.get('flags', {}).get('io_loop_back', False)), // Removed in V2
        ...
        extra_flags = {
            'io_od_mode': 0, # Not applicable for RX
        }
